...
func (user *User) Session() (세션 Session, 오류 error) {
	세션 = Session{}
	오류 = DB.QueryRow("SELECT id, uuid, email, user_id, created_at FROM sessions WHERE user_id = $1", user.Id).
		Scan(&세션.Id, &세션.Uuid, &세션.Email, &세션.UserId, &세션.CreatedAt)
	return
}

func (세션 *Session) Check() (유효 bool, 오류 error) {
	오류 = DB.QueryRow("SELECT id, uuid, email, user_id, created_at FROM sessions WHERE uuid = $1", 세션.Uuid).
		Scan(&세션.Id, &세션.Uuid, &세션.Email, &세션.UserId, &세션.CreatedAt)
	if 오류 != nil {
		유효 = false
		return
